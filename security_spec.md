...
- **Identity Integrity**: All documents (`/habits`, `/habitLogs`, `/tasks`) must have a `userId` field that strictly matches the creator's UID.
...
| 7 | Create | `/users/V1` | `{ "email": "me@me.com", "isAdmin": true }` | Privilege Escalation |
| 8 | Delete | `/habits/H1` | - (by non-owner) | Prevent Malicious Deletion |
| 9 | Update | `/users/U1` | `{ "latestEnergyLevel": 5 }` (by non-owner) | PII Isolation |
...
All the above payloads must return `PERMISSION_DENIED` in the Firestore Security Rules emulator.
